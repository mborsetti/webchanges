...
# ruff: noqa: S301
"""
Vendored version of tests for packaging.version.parse() from packaging v26.3 released on 03-Aug-26
(code https://github.com/pypa/packaging/blob/929fd4b1410ac7ef61ef3f45b2f5d7e87711a9b5/tests/test_version.py).

We make the "pretend" library optional, and skip the pickle backward-compatibility tests because their
embedded pickle bytes reference ``packaging.version.Version`` rather than the vendored module path.

The ``S301`` (``pickle.loads`` security warning) check is silenced file-wide because the pickle round-trip
tests deserialize bytes generated within the test itself.

Lives under ``tools/`` rather than ``tests/`` because the parser only needs verification when *it* changes -- not on
every release run. Run with::

    pytest tools/test_packaging_version.py.
"""
...
import sys
import typing
from copy import replace  # ty:ignore[unresolved-import]
...
from webchanges._vendored.packaging_version import (  # noqa: E402
    Infinity,
    InvalidVersion,
    NegativeInfinity,
    Version,
...
try:
    import pretend  # ty:ignore[unresolved-import]
except ImportError:
    pretend = None
...
class TestVersion:
    ...
    def test_non_string_versions_raise_invalid_version(self, version: object) -> None:
        with pytest.raises(InvalidVersion):
            Version(version)  # type: ignore[arg-type]  # ty:ignore[invalid-argument-type]
    ...
    @pytest.mark.parametrize(
        ...
        list(
            itertools.chain.from_iterable(
                ...
                [[(x, y, operator.ne) for j, y in enumerate(VERSIONS) if i != j] for i, x in enumerate(VERSIONS)]
                +
    ...
    def test_comparison_true(self, left: str, right: str, op: Callable[[Version, Version], bool]) -> None:
    ...
    @pytest.mark.parametrize(
        ...
        list(
            itertools.chain.from_iterable(
                ...
                +
                # Verify that the equal (==) operator works correctly
                [[(x, y, operator.eq) for j, y in enumerate(VERSIONS) if i != j] for i, x in enumerate(VERSIONS)]
                +
                # Verify that the not equal (!=) operator works correctly
    ...
    def test_comparison_false(self, left: str, right: str, op: Callable[[Version, Version], bool]) -> None:
    ...
    @pytest.mark.skipif(pretend is None, reason='requires the optional "pretend" package')
    @pytest.mark.parametrize(('op', 'expected'), [('eq', False), ('ne', True)])
    def test_compare_other(self, op: str, expected: bool) -> None:
        other = pretend.stub(**{f'__{op}__': lambda _: NotImplemented})  # ty:ignore[unresolved-attribute]

        assert getattr(operator, op)(Version('1'), other) is expected

    @pytest.mark.parametrize('op', ['__lt__', '__le__', '__eq__', '__ge__', '__gt__', '__ne__'])
    ...
    def test_replace_invalid_release_tuple_content(self) -> None:
        v = Version('1.2.3')
        with pytest.raises(InvalidVersion, match='release must be a non-empty tuple of non-negative'):
    ...
    def test_replace_invalid_local_string(self) -> None:
        v = Version('1.2.3')
        with pytest.raises(InvalidVersion, match='local must be a valid version string'):
...
def reset_version_parts(version: Version, **kwargs: typing.Any) -> None:
...
def test_from_parts_rejects_non_str_pre_letter() -> None:
    ...
    with pytest.raises(InvalidVersion, match='pre must be a tuple'):
        Version.from_parts(release=(1,), pre=(1, 1))  # type: ignore[arg-type]  # ty:ignore[invalid-argument-type]
...
@pytest.mark.skip(reason='pickle bytes reference packaging.version.Version, not the vendored module path')
