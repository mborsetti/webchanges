...
.. code-block:: bash
   ...
   uv pip install git+https://github.com/mborsetti/webchanges.git@unreleased
...
* New ``!env`` YAML tag: inserts the value of environment variables anywhere within a string value of the jobs or
  ...
  in multiple URLs and names, which can even be computed by a wrapper script before invoking :program:`webchanges`)
...
* New ``between`` filter: extracts the block of lines sitting between a line matching the ``start`` regular
  ...
  of log files, server-status pages, or any text delimited by recognizable markers. See :ref:`here <between>`.
* ``deepdiff`` differ: new ``cutoff_distance_for_pairs``, ``cutoff_intersection_for_pairs`` and
  ...
  compares them in depth rather than reporting the whole object as replaced. See :ref:`here <deepdiff_diff>`.
* New ``same_site_delay`` job directive for ``url`` and browser (``use_browser: true``) jobs: when an earlier job in
  ...
  be set on an individual job or, for all jobs, through ``job_defaults``. See :ref:`here <same_site_delay>`.
...
* ``command`` jobs: when :program:`webchanges` is run verbosely (``-v``, ``-vv`` or ``-vvv``), the matching verbosity
  flag is now passed on to any Python interpreter invoked by the command, so that the script's own logging shows up
  alongside that of :program:`webchanges` (e.g. when running ``webchanges -vv``, a job with ``command: python
  summarize.py`` is run as ``python summarize.py -vv``). The flag is added at the end of the invocation, where the
  script being run will see it, and a verbosity flag you wrote yourself is never removed or lowered. Note that a
  Python script which does not accept a ``-v`` argument may now fail when :program:`webchanges` is run verbosely
  (including with ``--log-file``, which implies ``-v``). See :ref:`here <python_command_verbosity>`.
* New ``version_check`` configuration setting: set its ``enabled`` sub-directive to ``false`` in ``config.yaml`` to
  disable the check against PyPi for a newer release of :program:`webchanges` whose result appears in report footers
  (e.g. for air-gapped machines or installations managed by a package manager). The ``--check-new`` command line
  argument always queries PyPi, regardless of this setting. See :ref:`here <config_version_check>`.
...
* The check for a newer release of :program:`webchanges` on PyPi (whose result appears in report footers and in the
...
* ``suppress_repeated_errors`` job directive: a job failing repeatedly with the same error was reported on every run
  ...
  Reported by `Manu <https://github.com/m3nu>`__ in `#189 <https://github.com/mborsetti/webchanges/issues/189>`__.
...
Internals
`````````
* GitHub Release notes are now published in Markdown (``RELEASE.md``), auto-generated from ``RELEASE.rst`` by a
  pre-commit hook.
