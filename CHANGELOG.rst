...
----------------------------------------
Jobs with``use_browser: true`` to use Playwright (instead of Pyppeteer)
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
...
* is not vetted for security (TLS certificates were disabled when downloading browsers!)
...
The `open issues <https://github.com/pyppeteer/pyppeteer/issues>`__ now
exceed 100.

As a result, I have been investigating a substitute, and found one in `Playwright <https://playwright
.dev/python/>`__, in combination with the latest stable version of Google Chrome. This package has none of the
issues above and is supported by the might of Microsoft, who has been keeping it up-to-date.
...
* The experimental ``block_elements`` sub-directive is not implemented (yet?) and is ignored.
...
* ``headless`` (true/false): Launch browser in headless mode (i.e. invisible) (defaults to true). Set to false to see
...
Changed
-------
* The method ``bs4`` of filter ``html2text`` now can control the ``strip`` parameters, and now conforms to
  BeautifulSoup's default of false as it gives better output in most cases. To restore previous behavior, add the
  ``strip: true`` sub-directive of the ``html2text`` filter to impacted jobs.

Added
-----
* The ``html`` report type has a new job directive ``monospace`` forcing the output to use a monospace font.
...
* The ``email`` reporter with the ``sendmail`` method sub-directive will pass the ``from`` sub-directive (when
  specified) as a ``-f`` command line argument to the ``sendmail`` executable. Contributed by
