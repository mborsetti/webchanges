...
class BrowserLoop(object):
    ...
    async def _launch_browser(self, chromium_revision, proxy_server, ignore_https_errors, user_data_dir, switches):
        ...
        logger.info(
            f"os.environ.get('PYPPETEER_CHROMIUM_REVISION')=" f"{os.environ.get('PYPPETEER_CHROMIUM_REVISION')}"
        )
    ...
    async def _get_content(
        self, url, headers, cookies, timeout, proxy_username, proxy_password, wait_until, wait_for, wait_for_navigation
    ):
...
class BrowserContext(object):
        ...
        with BrowserContext._BROWSER_LOCK:
            if BrowserContext._BROWSER_REFCNT == 0:
                ...
                BrowserContext._BROWSER_LOOP = BrowserLoop(
                    chromium_revision, proxy_server, ignore_https_errors, user_data_dir, switches
                )
    ...
    def process(
        ...
        return BrowserContext._BROWSER_LOOP.process(
            url, headers, cookies, timeout, proxy_username, proxy_password, wait_until, wait_for, wait_for_navigation
        )
...
def main():
    ...
    try:
        ctx = BrowserContext(
            args.chromium_revision, args.ignore_https_errors, proxy_server, args.user_data_dir, args.switches
        )
        print(
            ctx.process(
                args.url,
