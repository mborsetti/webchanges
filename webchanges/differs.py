...
class UnifiedDiffer(DifferBase):
    ...
    __supported_directives__ = {
        'rangeinfo': 'include range information lines (default: true)',
    }
    ...
    def differ(
        ...
        if report_kind == 'html' and _unfiltered_diff is not None and 'text' in _unfiltered_diff:
            ...
            if directives.get('rangeinfo') is False or (
                directives.get('rangeinfo') is None and self.job.additions_only and (len(diff) < 4 or diff[3][0] != '/')
            ):
...
class AIGoogleDiffer(DifferBase):
    ...
    __supported_directives__ = {
        ...
        'context_lines': 'the number of context lines for {unified_diff} (default: 9999)',
    ...
    def differ(
        ...
        def get_ai_summary(prompt: str) -> str:
            ...
            if '{unified_diff}' in prompt:
                context_lines = directives.get('context_lines', 9999)
                unified_diff = '\n'.join(
                    difflib.unified_diff(
                        str(self.state.old_data).splitlines(),
                        str(self.state.new_data).splitlines(),
        ...
        DifferBase.process('unified', {}, self.state, 'text', tz, temp_unfiltered_diff)
        unified_report = DifferBase.process('unified', {}, self.state, 'html', tz, temp_unfiltered_diff)
        return {
            'text': summary + '\n\n' + unified_report['text'] + '\n------------\n' + footer,
