...
class HtmlReporter(ReporterBase):
    ...
    def mark_to_html(self, text: str, markdown_padded_tables: bool | None = False) -> str:
        """Converts a line of Markdown (e.g. as generated by html2text filter) back to html.
        ...
        html_out, sub = self.re_ptags.subn('', html_out)
        if sub:
            return pre + html_out + post
        html_out = self.re_htags.sub(r'<\g<1>strong>', html_out)  # replace heading tags with <strong>
        return pre + html_out + post

    def _diff_to_html(self, diff: str, job: JobBase) -> Iterator[str]:
            ...
            if job.diff_tool.startswith('wdiff'):
                ...
                if job.is_markdown:
                    ...
                    for line in diff.splitlines():
                        ...
                        line = self.mark_to_html(line, job.markdown_padded_tables)
                        if is_add:
                            line = '{+' + line
                            is_add = False
                        elif is_del:
                            ...
                            is_del = False
                        loc = 0
                        while line.find('{+', loc) > 0 or line.find('[-', loc) > 0:
                            if line.find('{+', loc) != -1 and (
                                line.find('{+', loc) < line.find('[-', loc) if line.find('[-', loc) != -1 else True
                            ):
                                if line.find('+}', loc) == -1:
                                    if self.re_tagend.search(line):
                                        line = self.re_tagend.sub(lambda x: '+}' + x.group(0), line)
                                    else:
                                        line += '+}'
                                    is_add = True
                                    break
                ...
                diff = re.sub(
                    r'\{\+(.*?)\+}',
                    lambda x: f'<span style="{added_style}">{x.group(1)}</span>',
                    diff,
                    flags=re.DOTALL,
                )
                diff = re.sub(
                ...
                #         yield mark_to_html(line).replace('</strong></span>', '</span></strong>')
                # else:
                yield diff
                if job.monospace:
                    yield '</span>'
            ...
            if not job.monospace:
                ...
                for i, line in enumerate(diff.splitlines()):
                    ...
                    if (
                        ...
                        if job.is_markdown:
                            yield f'<tr{style}><td>{self.mark_to_html(line[1:], job.markdown_padded_tables)}</td></tr>'
                        else:
